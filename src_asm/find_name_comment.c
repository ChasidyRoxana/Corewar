...
{/* зачем двойной указатель на гнл?*/
	int i;
    ...
    // переходим на след строку и снова записываем/проверяем
	else
	{
		*tmp = (*tmp)->next;
...
int		find_name(char *name, t_asm *asmb, t_gnl **tmp)
{
	...
	{
		while (name[i] && name[i] != '\"')
		{/* можно в условии вайла j < PROG_NAME_LENGTH, чтобы не вызывать каждый раз функцию*/
			// чекаем, не переполнен ли prog_name
            if (length_error(j, PROG_NAME_LENGTH))
				return (0);
            // записываем
			asmb->header.prog_name[j++] = name[i++];
		}/* а после проверить, что вышли из цикла не из-за переполнения и обработать ошибку*/
